...
#include <bits/stdc++.h>
using namespace std;
int index(vector<int> &input)
{ // i can also return index of minimum element in SRA
    if (input.size() == 1)
    {
        return input[0];
    }
    ...
    {                     // check condition whether array is sorted
...
int main()
{
    ...
    cout << "minimum element is " << index(v) << endl;
    return 0;
}