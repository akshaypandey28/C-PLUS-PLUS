...
int rectanglesum(vector<vector<int>> &v, int L1, int R1, int L2, int R2)
{
  ...
  for (int i = 0; i < v.size(); i++){
    ...
    for (int j = 1; j < v[0].size(); j++)  v[i][j] = v[i][j] + v[i][j - 1];
    
  }
  cout << "PREFIX SUM MATRIX row wise IS " << endl; // row wise

  for (int i = 0; i < v.size(); i++){
    for (int j = 0; j < v[0].size(); j++) cout << v[i][j] << " "; // row wise
    cout << endl;
  }

  // prefix sum array column-wise
  for (int i = 1; i < v.size(); i++){
    for (int j = 0; j < v[0].size(); j++)v[i][j] = v[i][j] + v[i - 1][j];
  }

  cout << "PREFIX SUM MATRIX column wise IS " << endl; // column wise
  for (int i = 0; i < v.size(); i++){
    for (int j = 0; j < v[0].size(); j++) cout << v[i][j] << " "; // column wise
    cout << endl;
  }
