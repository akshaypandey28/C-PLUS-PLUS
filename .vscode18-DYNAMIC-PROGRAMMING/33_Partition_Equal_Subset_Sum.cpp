...
public:
    ...
    bool canPartition(vector<int>& nums) {
        ...
        //means size kitna bhi ho that is n 0 sum always possible hoga that is row is size and col is sum 
        //1st col is representing 0 sum with size n [0,n-1]
        for(int i = n-1; i >= 0; i--) {  //this is used for picking the elements from the nums array
            for(int j = 1; j <= k; j++) { //this represents sum from 1 to k that is col 1 to k
                if(nums[i] <= j)
